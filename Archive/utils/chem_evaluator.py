...
def is_valid_smiles(smiles):
    """
    Check if a SMILES string is valid by trying to create an RDKit molecule object and sanitizing it.
    Args:
        smiles (str): SMILES string.
    Returns:
        bool: True if valid and can be sanitized, False otherwise.
    """
    ...
    if mol is None:
        ...
        try:
            ...
            print(f"Sanitization failed for {smiles}, invalid chemistry.")
            return False
        except Exception as e:
            print(f"Error sanitizing SMILES '{smiles}': {str(e)}")
            return False

def fcd_distance_torch(generated_smiles_lst, training_smiles_lst):
    """
    Evaluate FCD distance between generated and training SMILES sets using PyTorch.
    Args:
        generated_smiles_lst (list): List of generated SMILES strings.
        training_smiles_lst (list): List of training SMILES strings.
    Returns:
        float: FCD distance.
    """
    ...
    valid_generated_smiles = [smi for smi in generated_smiles_lst if is_valid_smiles(smi)]
    valid_training_smiles = [smi for smi in training_smiles_lst if is_valid_smiles(smi)]

    if len(valid_generated_smiles) == 0 or len(valid_training_smiles) == 0:
        raise ValueError("Insufficient valid SMILES for FCD calculation")

    fcd = FCD(device="cuda", n_jobs=1)
    fcd_distance = fcd(valid_generated_smiles, valid_training_smiles)
    return fcd_distance
