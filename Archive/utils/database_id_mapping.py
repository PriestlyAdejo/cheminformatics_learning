    ...
    def map_ids(self, from_db, to_db, ids):
        ...
        if job_status:
            ...
            for original_id in ids:
                filtered_results_df = results_df[results_df['from'] == original_id]
                mapped_id = filtered_results_df.iloc[0]['to'] if not filtered_results_df.empty else "No ID found"
                conversion_dict[original_id] = mapped_id
            conversion_dict = {k: v if v.lower() != 'no id found' else None for k, v in conversion_dict.items()}
