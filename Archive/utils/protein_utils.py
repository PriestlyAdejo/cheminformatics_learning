...
def featurize_as_graph(protein, num_rbf=16, device="cuda"):
    # Extracting coordinates and sequence information
    N_coords, CA_coords, C_coords, O_coords = [
        protein[protein.name == name][['x', 'y', 'z']].to_numpy() for name in ['N', 'CA', 'C', 'O']
    ]
    max_length = min(len(N_coords), len(CA_coords), len(C_coords), len(O_coords))
    coords = np.stack((N_coords[:max_length], CA_coords[:max_length], C_coords[:max_length], O_coords[:max_length]), axis=1)

    coords = torch.from_numpy(coords).float().to(device)
    seq = torch.tensor([_amino_acids(a) for a in protein[protein.name == 'CA']['resname'][:max_length]], dtype=torch.long).to(device)
    mask = torch.isfinite(coords.sum(dim=(1,2)))
    coords[~mask] = np.inf

    X_ca = coords[:, 1]
    edge_index = torch_cluster.radius_graph(X_ca, r=5.0, loop=False)
    pos_embeddings = positional_embeddings(edge_index, device=device)
    ...
    data = torch_geometric.data.Data(
        x=X_ca, seq=seq, node_s=node_s, node_v=node_v,
