...
from .helpers import pdb, nglview, io
...
class Protein:
    ...
    def __init__(self, identifier_type, identifier_value, protein_output_path, save_nglview=True):
        """
        Parameters
        ----------
        ...
        setattr(self, "save_nglview", save_nglview)
        ...
        if identifier_type is Consts.Protein.InputTypes.PDB_CODE:
            io.create_folder(Path(protein_output_path) / "protein" / f"PDB_{identifier_value}")
            self.pdb_filepath = pdb.fetch_and_save_pdb_file(
                identifier_value, Path(protein_output_path) / "protein" / f"PDB_{identifier_value}" / identifier_value
            )
            self.nglview_filepath = Path(protein_output_path) / "protein" / f"PDB_{identifier_value}" / f"nglview_viz_protein_PDB_{identifier_value}.png" 
    ...
    def __call__(self):
        ...
        if hasattr(self, "pdb_code"):
            if self.save_nglview:
                viewer = nglview.protein("pdb_code", self.pdb_code, self.nglview_filepath)
            else:
