...
# @redo.retriable(attempts=5, sleeptime=10)
...
def similarity_search(
    ...
    """
    escaped_smiles = quote(smiles).replace("/", ".")
    partial_url = APIConsts.URLs.Inputs.SIMILARITY_FROM_SMILES.value + escaped_smiles + "/"
    url = "".join(p + "/" for p in partial_url.split("/") if len(p) > 0)
