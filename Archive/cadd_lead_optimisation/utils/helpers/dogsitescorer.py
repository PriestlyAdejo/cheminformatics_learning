...
from . import pdb, io as helper_io
...
def save_binding_sites_to_file(binding_site_df, output_path):
    ...
    Each binding site pocket and its subpockets will have a unique directory.
    ...
    for binding_site in binding_site_df.index:
        ...
        target_output_path = get_binding_site_name_output_path(output_path, binding_site)

        for column in ["pdb_file_url", "ccp4_file_url"]:
            response = requests.get(binding_site_df.loc[binding_site, column])
            response.raise_for_status()
            file_extension = ".pdb" if column == "pdb_file_url" else ".ccp4"
            response_file_content = response.content if file_extension == ".pdb" else gzip.decompress(response.content)

            file_name = binding_site + file_extension
            with open(target_output_path / file_name, "wb") as f:
                f.write(response_file_content)
    return

def get_binding_site_name_output_path(binding_site_output_path, binding_site_name):
    ...
    helper_io.create_folder(pocket_output_path)
    if subpocket:
        subpocket_output_path = pocket_output_path / f"subpocket_{subpocket}"
        helper_io.create_folder(subpocket_output_path)
...
def calculate_pocket_coordinates_from_pocket_pdb_file(filepath):
    ...
    # Modified regex pattern to correctly capture negative numbers and floating points
    coordinates = [float(element) for element in coordinates_data_as_list if re.match(r'-?\d+\.\d+', element)]
    
    if len(coordinates) >= 4:
        pocket_coordinates = {
            "center": coordinates[:3],
            "size": [coordinates[3] * 2 for _ in range(3)],
        }
    else:
        raise ValueError("Insufficient coordinate data found in PDB file.")
