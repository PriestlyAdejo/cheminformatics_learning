...
def binding_site(protein_input_type, protein_input_value, ccp4_filepath, output_image_filename=None):
    """
...
def docking(
    ...
    protein_file_name = protein_filepath.stem
    
    for docking_pose_filepath, ligand_label, docking_pose_label in zip(
        list_docking_poses_filepaths, list_docking_poses_affinities, list_docking_poses_labels
    ):
        docking_pose_filepath = Path(docking_pose_filepath)
        output_image_filename = f"nglview_viz_protein_{protein_file_name}_ligand_{docking_pose_label.strip().replace('-', '_')}.png"
        output_image_filepath = docking_pose_filepath.parent / output_image_filename
        ...
        if output_image_filepath is not None:
            viewer.render_image(trim=True, factor=2)
            viewer._display_image()
            viewer.download_image(output_image_filepath)
    ...
    selector = Select(
        ...
        rows=min(len(list_docking_poses_filepaths), 52),
    ...
    def _on_selection_change(change):
        if change["name"] == "value" and (change["new"] != change["old"]):
            viewer.hide(range(1, len(list_docking_poses_filepaths) + 1))
...
# need to create a folder for each dockiong pose etc like done in the above docking fuiction
def interactions(
    protein_filepath,
    list_docking_poses_filepaths,
    list_docking_poses_labels,
