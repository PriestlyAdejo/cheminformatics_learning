...
from gvp.models import TransformerGVP
...
class BindingAffinityPredictor(pl.LightningModule):
    ...
    def test_step(self, test_batch, batch_idx):
        ...
        # Check if the 'y' attribute exists and is not None
        if hasattr(test_batch[0], 'y') and test_batch[0].y is not None:
            y_true = test_batch[0].y.cpu()
        ...
        if y_true is not None:
            ...
            # Log metrics
            self.log_dict({"MSE": results["mse"], "RM2": results["rm2"], "CI": results["ci"]}, prog_bar=True, on_step=False, on_epoch=True)
        else:
            # Handle predictions without true labels
            self.predictions.append(y_pred.numpy())

            # Since we don't have true labels, we create a dummy result dictionary
            results = {"predictions": y_pred.numpy()}
